...
export const SFTTrainingNode = memo(({ data, isConnectable }: NodeProps<LLMNodeData>) => {
  const hasErrors = data.connectionErrors && data.connectionErrors.length > 0;
  
  return (
    <div className="bg-indigo-900 p-4 rounded-md border-2 shadow-md w-64">
      ...
      <div className="text-xs">
        ...
        <div className="flex justify-between mb-1">
          <span>Optimizer:</span>
          <span>{String(data.params.optimizer || 'adamw')}</span>
        </div>
      </div>
...
SFTTrainingNode.displayName = 'SFTTrainingNode'; 