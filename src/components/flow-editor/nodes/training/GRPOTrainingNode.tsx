...
export const GRPOTrainingNode = memo(({ data, isConnectable }: NodeProps<LLMNodeData>) => {
  const hasErrors = data.connectionErrors && data.connectionErrors.length > 0;
  
  return (
    <div className="bg-purple-900 p-4 rounded-md border-2 shadow-md w-64">
      ...
      <div className="text-xs text-slate-300 mb-2">
        Generalized Reward-Penalized Optimization (GRPO) Training
      </div>
      ...
      <div className="text-xs">
        ...
        <div className="flex justify-between mb-1">
          ...
          <span>{String(data.params.rewardThreshold)}</span>
        </div>
      </div>
...
GRPOTrainingNode.displayName = 'GRPOTrainingNode'; 