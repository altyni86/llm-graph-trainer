...
export const QKVAttentionNode = memo(({ data, isConnectable }: NodeProps<LLMNodeData>) => {
  ...
  return (
    <div className="bg-slate-800 p-4 rounded-md border-2 shadow-md w-64">
      ...
      <div className="text-xs">
        ...
        <div className="flex justify-between mb-1">
          <span>Num Heads:</span>
          <span>{String(data.params.numHeads)}</span>
        </div>
        <div className="flex justify-between mb-1">
          <span>Attention Type:</span>
          <span>{String(data.params.attentionType || 'scaled_dot_product')}</span>
        </div>
      </div>
...
QKVAttentionNode.displayName = 'QKVAttentionNode'; 